...
import { PageView, TtsVoiceOption, TtsLanguageOption, TtsResultData } from '../types.js';
import {
  Mic,
  Volume2,
  VolumeX,
...
const SAMPLE_PRESETS: { label: string; text: string; language: string; voice: string }[] = [
  {
    ...
    language: 'en',
    voice: 'Charon',
    text: 'Welcome to Convert-X. Our online suite allows you to transform, compress, and generate high-fidelity media across all major formats with zero retention and maximum security.',
  },
  {
    label: 'हिन्दी – स्वागत और ऑडियो विवरण (Hindi)',
    language: 'hi',
    voice: 'Kore',
    text: 'कन्वर्ट-एक्स में आपका स्वागत है। अपने लिखे हुए पाठ को तुरंत सहज, स्पष्ट और स्वाभाविक आवाज़ में बदलें और उच्च गुणवत्ता वाला ऑडियो डाउनलोड करें।',
  },
  {
    label: 'اردو – قدرتی اور واضح آواز (Urdu)',
    language: 'ur',
    voice: 'Fenrir',
    text: 'کنورٹ ایکس میں خوش آمدید۔ اپنی تحریر کو جدید ترین آوازوں کے ذریعے قدرتی اور پرکشش آڈیو میں تبدیل کریں اور باآسانی ڈاؤن لوڈ کریں۔',
  },
  {
    label: 'Español – Narración Profesional (Spanish)',
    language: 'es',
    voice: 'Zephyr',
    text: 'Bienvenido a Convert-X. Transforma cualquier texto en una locución natural y descarga tu archivo de audio con la máxima claridad y rapidez.',
  },
];
...
export const TextToVoicePage: React.FC<TextToVoicePageProps> = ({
  ...
  const [text, setText] = useState<string>(
    'Welcome to Convert-X. Turn your text into natural-sounding speech and download the audio.'
  );
  ...
  const handleGenerateVoice = async () => {
    ...
    try {
      ...
      setResult(ttsData);
      setIsGenerating(false);
      showToast('Voice generated successfully!', `Synthesized ${ttsData.wordCount} words in ${ttsData.durationSeconds}s.`, 'success');
