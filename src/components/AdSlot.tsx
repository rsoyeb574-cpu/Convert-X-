...
export const AdSlot: React.FC<AdSlotProps> = ({
  ...
  const resolvedSlotId = slotId || (position === 'primary' ? 'convertx-primary-slot' : 'convertx-secondary-slot');
  ...
  // When AdSense is not filled / not configured, collapse or display standard reserved container
  return (
    <div
      ref={adRef}
    ...
    >
      <div className="w-full max-w-4xl rounded-2xl border border-dashed border-[#CBD5E1] dark:border-[#334155] bg-slate-50/50 dark:bg-[#0F172A]/40 px-4 py-4 sm:py-5 text-center transition-colors">
        ...
        <p className="text-[11px] sm:text-xs text-[#64748B] dark:text-[#94A3B8] max-w-md mx-auto">
          Reserved Google AdSense ad slot. Upgrade to Pro for an ad-free experience.
        </p>
      </div>
    </div>
  );
